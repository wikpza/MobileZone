...
import DialogReportLoanPayment from "@/pages/admin/ReportPage/DialogReportLoanPayment.tsx";
...
const MakeReportManufacturing = ({signature, employee}:Props) => {
    ...
    if(loanList?.status && (loanList.status === 401 || loanList.status === 403 ) ) return (<AccessDeniedPage/>)
    if(!loanList?.loans || !loanList) return (<div>unable to load data</div>)
