...
import {months} from "@/lib/utils.ts";
import DialogReportSalary from "@/pages/admin/ReportPage/DialogReportSalary.tsx";
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 ">
                    <div className="flex flex-col gap-2">
                        <label htmlFor="month-select"
                               className={`text-sm font-medium ${errorOption.month !== "" && 'text-red-700'}`}>Month</label>
                        <Select value={dateOption.month.toString()}
                                onValueChange={(value) => setDateOption({...dateOption, month: Number(value)})}
                        >
                            <SelectTrigger id="month-select"
                                           className={` w-full ${errorOption.month !== "" && "border-red-700 border text-red-700"}`}>
                    ...
                    <div className="flex flex-col gap-2">
                        <label htmlFor="year-select"
                               className={`text-sm font-medium ${errorOption.year !== "" && 'text-red-700'}`}>Year</label>
                    ...
                    <DialogReportSalary data={data?.data} signature={signature} employee={employee}/>
