...
import { useUpdateRawMaterial} from "@/api/RawMaterial.api.ts";
import {QueryObserverResult, RefetchOptions, RefetchQueryFilters} from "react-query";
import {GetMaterial, Material} from "@/types/material.ts";
import {isFormErrors} from "../../../../../../../../main/Compass Contract/frontend/src/lib/errors";
import {toast} from "sonner";
import {GetUnit} from "@/types/unit.ts"; // Ваши компоненты формы
...
const MyForm = ({refetch, material, units}:Props) => {
    const {updateMaterial, isSuccess, response} = useUpdateRawMaterial()

    const [open, setOpen] = React.useState(false);
    const methods = useForm<FormData>({
        resolver: zodResolver(formSchema),
        defaultValues:{name:material.name, unitId:material.unit.id}
    });
    ...
    const onSubmit = (data: FormData) => {
        if(data.name === material.name && data.unitId === material.unit.id){
            methods.setError("name", {
                type: "manual",
    ...
    return (
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <div className="p-4">
                    <h2 className="text-lg font-semibold mb-4">Create Raw Material</h2>
