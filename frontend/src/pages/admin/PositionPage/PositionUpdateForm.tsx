...
import {useCreatePosition, useGetPositions, useUpdatePosition} from "@/api/position.api.ts";
...
const PositionUpdateForm = ({refetch, position}:Props) => {
    const {updateUnit, isSuccess, response} = useUpdatePosition()
    ...
    const onSubmit = (values: CreatePositionType) => {
        ...
        if(methods.getValues('name') === position.name){
            methods.setError("name", {
                type: "manual",
                message: "you haven't changed anything",
            });
        }else{
            updateUnit({id:position.id.toString(), ...values})
        }
    };
        ...
        <Dialog open={open} onOpenChange={setOpen}>
            ...
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription>
                        The dialog helps you to edit position.
                    </DialogDescription>
                </DialogHeader>
                <div>
                    <Form {...methods}>
