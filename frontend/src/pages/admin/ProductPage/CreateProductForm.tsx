...
import {useGetAdminUnits} from "@/api/Unit.api.ts";
...
import {useCreateProduct} from "@/api/Product.api.ts";
...
const MyForm = ({ refetch, units }: Props) => {
    const { create, isSuccess, response } = useCreateProduct();
    ...
    if (units.units.length === 0) {
        return <div>before adding product, add unit</div>;
    }
